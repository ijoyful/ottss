	...
	<script type="text/javascript">
		function deleteOk() {
			if(confirm('게시글을 삭제 하시 겠습니까 ? ')) {
				...
				let url = '${pageContext.request.contextPath}/admin/freeboard/delete?' + query;
				location.href = url;
			}
		}
	</script>
...
<body>
								...
								<td colspan="2">
									<c:forEach var="vo" items="${listFile}" varStatus="status">
										<p class="border text-secondary mb-1 p-2">
											...
											<a href="${pageContext.request.contextPath}/admin/freeboard/download?fileNum=${vo.fileNum}">${vo.c_fileName}</a>
										</p>
									</c:forEach>
								</td>
							</tr>
							<tr>
								<td colspan = "2"> 다음글 :
									<c:if test="${not empty nextDTO}">
										<a href="${pageContext.request.contextPath}/admin/freeboard/article?${query}&fb_num=${nextDTO.fb_num}">${nextDTO.title}</a>
									</c:if>
								</td>
							</tr>
							<tr>
								<td colspan = "2"> 이전글 :
									<c:if test="${not empty prevDTO}">
										<a href="${pageContext.request.contextPath}/admin/freeboard/article?${query}&num=${prevDTO.fb_num}">${prevDTO.title}</a>
									</c:if>
								</td>
							</tr>
						</tbody>
					...
					<table class="table table-borderless">
						<tr>
							<td width="50%">
								 <c:choose>
	                            	<c:when test="${sessionScope.member.id==dto.id}">                                                                    
	                            		<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/admin/freeboard/update?fb_num=${dto.fb_num}&page=${page}';">수정</button>
	                           		</c:when>
	                           		<c:otherwise>
							...
							<td class="text-end">
								<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/admin/freeboard/list?${query}';">리스트</button>
							</td>
						</tr>
					</table>
	...
	<script type="text/javascript">
		function login() {
			location.href = '${pageContext.request.contextPath}/admin/freeboard/login';
		}
		...
		$(function() {
			$('.btnSendReply').click(function() {
				...
				let url = '${pageContext.request.contextPath}/admin/freeboard/insertComment';
		...
		function listPage(page) {
			let url = '${pageContext.request.contextPath}/admin/freeboard/listComment';
		...
		$(function() {
			$('#listReply').on('click','.deleteReply', function(){
				...
				let url = '${pageContext.request.contextPath}/admin/freeboard/deleteComment';
		...
		$(function() {
			$('.btnSendBoardLike').click(function() {
				...
				let url = '${pageContext.request.contextPath}/admin/freeboard/insertFreeLike';
		...
		$(function(){
			...
			myModalEl.addEventListener('hidden.bs.modal', function(){
				// 모달 대화상자가 닫힐때
				location.href = '${pageContext.request.contextPath}/admin/freeboard/article?${query}&num=' + fb_num;
			});
		});
		
		function reportdialogshow() {
			let url = '${pageContext.request.contextPath}/admin/freeboard/report';
			let num = ${dto.fb_num};
			const fn = function(data) {
